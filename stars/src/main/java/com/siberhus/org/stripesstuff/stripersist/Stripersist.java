...
public class Stripersist implements Interceptor, ConfigurableComponent {
    ...
    public static EntityManagerFactory getEntityManagerFactory() {
        if (Stripersist.entityManagerFactories.size() != 1) {
        	EntityManagerFactory emf = Stripersist.entityManagerFactories.get("default");
        	if(emf!=null) return emf;
            StripesRuntimeException sre = new StripesRuntimeException(
                    "In order to call Stripersist.getEntityManagerFactory() without any parameters there must be exactly one persistence unit defined.");
        ...
        return Stripersist.entityManagerFactories.values().iterator().next();
    }
